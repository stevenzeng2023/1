...
def huawei_model(host, username, password, su, cmds):
    try:
        with Telnet(host, port=23, timeout=3) as telnet:
            # 判断是否华为或者思科交换机，并输入aaa账号密码
            if telnet.read_until(b'Username:', timeout=1):
                index1, obj1, output1 = telnet.expect([b'Username:', b'Password:'], timeout=1)
                if index1 == 0:
            ...
            #  判断是否华三交换机，并输入aaa账号密码
            elif telnet.read_until(b'Login:', timeout=1):
                index, obj, output = telnet.expect([b'Login:', b'Password:'], timeout=1)
                if index == 0:
                    telnet.write(username.encode('ascii') + b'\n')
                    telnet.read_until(b'Password:', timeout=1)
                    telnet.write(password.encode('ascii') + b'\n')
            ...
            telnet.read_until(b'>', timeout=1)
            telnet.write(b'sys' + b'\n')
            index, a, b = telnet.expect([b'>', b']'], timeout=1)
            if index == 0:
                telnet.write(b'su' + b'\n')
                telnet.read_until(b'Password:', timeout=1)
                telnet.write(su.encode() + b'\n')
